...
import { Trash2, Edit2, Plus, Search, Folder, Layers, Activity } from "lucide-react";
...
export default function CurrentAffairsPage() {
    ...
    const handleToggleActive = (id: string, currentStatus: boolean) => {
        setConfirmTitle(currentStatus ? "Deactivate this Current Affair?" : "Activate this Current Affair?");
        setConfirmMessage(
            `Are you sure you want to ${currentStatus ? "deactivate" : "activate"} this Current Affair?`
        );
        ...
        setConfirmAction(() => async () => {
            try {
                ...
                toast.success(`Current Affair has been ${!currentStatus ? "activated" : "deactivated"}`);
            } catch (err) {
                console.error(err);
    ...
    const filteredAffairs = currentAffairs.filter((item) => {
        const matchesStatus =
            filterStatus === "true" ? item.active : filterStatus === "false" ? !item.active : true;
        const matchesCategory = filterCategory ? item.category?._id === filterCategory : true;
        const matchesSubCategory = filterSubCategory ? item.subCategory?._id === filterSubCategory : true;
        const matchesTitle = item.title.toLowerCase().includes(searchTitle.toLowerCase());
        return matchesStatus && matchesCategory && matchesSubCategory && matchesTitle;
    });
                    ...
                    <div className="flex flex-wrap gap-3 items-center bg-white p-4 rounded-xl shadow-sm mb-3">
                        ...
                        <div className="relative min-w-[150px]">
                            ...
                            >
                                ...
                                {Array.from(new Set(currentAffairs.map((c) => c.category?.name)))
                                    .filter(Boolean)
                                    .map((catName) => {
                                        const cat = currentAffairs.find((c) => c.category?.name === catName)?.category;
                                        return cat ? <option key={cat._id} value={cat._id}>{cat.name}</option> : null;
                                    })}
                            </select>
                        </div>
                        ...
                        <div className="relative min-w-[150px]">
                            ...
                            >
                                ...
                                {Array.from(new Set(currentAffairs.map((c) => c.subCategory?.name)))
                                    .filter(Boolean)
                                    .map((subName) => {
                                        const sub = currentAffairs.find((c) => c.subCategory?.name === subName)?.subCategory;
                                        return sub ? <option key={sub._id} value={sub._id}>{sub.name}</option> : null;
                                    })}
                            </select>
                        </div>
                    ...
                    <table className="w-full border-collapse bg-white rounded-2xl shadow-lg overflow-hidden">
                        <thead className="bg-gray-100 text-gray-700 text-sm uppercase tracking-wide font-semibold">
                            <tr>
                                ...
                                <th className="py-4 px-5 text-left border-b border-gray-200">Sub Category</th>
                                    ...
                                    <tr key={item._id} className="hover:bg-gray-50 transition-colors border-b border-gray-200">
                                        ...
                                        <td className="py-3 px-5 text-center">
                                            <label className="relative inline-flex items-center cursor-pointer">
                                                ...
                                                <div
                                                    ...
                                                    className={`w-12 h-6 flex items-center rounded-full p-1 transition-colors duration-300 cursor-pointer ${item.active ? "bg-green-500" : "bg-gray-300"
                                                        }`}
                                                >
                                                    <span
                                                        className={`bg-white w-5 h-5 rounded-full shadow-md transform transition-transform duration-300 ${item.active ? "translate-x-6" : "translate-x-0"
                                                            }`}
                                                    ></span>
                                                </div>
                                            </label>
                                        ...
                                        <td className="py-3 px-5 text-center">
                                            <div className="flex justify-center gap-2">
                                                <button
                                                    onClick={() => router.push(`/admin/current-affairs/${item._id}`)}
